...
def archive_recovered_temp(temp_path: Path):
    ...
    try:
        if temp_path and temp_path.exists():
            ...
            print(f"    ↳ archived recovered temp → {dest}")
    except Exception as e:
        print(f"    ⚠️  Could not archive recovered temp {temp_path}: {e}")
...
def merge_wal_in_place(dst_db: Path) -> bool:
    ...
    try:
        ...
        print(f"    ⚠️  WAL merge failed for {dst_db.name}: {e}")
        return False
    finally:
...
def try_sqlite_dissect(db_path: Path) -> Path | None:
    ...
    if not dissect_path or not dissect_path.exists():
        print(f"    ⚠️  sqlite_dissect CLI not found (looked for {SQLITE_DISSECT_PATH}).")
        return None
    ...
    try:
        ...
        print(f"    ⚠️  sqlite_dissect failed ({e.returncode}): {e.stderr[:300]}")
    except Exception as e:
        print(f"    ⚠️  sqlite_dissect error: {e}")
    return None
