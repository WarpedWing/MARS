...
from schema_manager import compare_schema
from schema_manager import load_all_schemas
from schema_manager import load_schema_catalog
from schema_manager import save_missing_schema_report
...
def looks_like_unix_timestamp(value):
    ...
    if not isinstance(value, (int, float)):
        return False
    return 946684800 <= value <= time.time() + 5 * 365 * 24 * 3600  # between 2000 and ~5 years ahead
...
def match_powerlog_schema(table_names: list[str]) -> tuple[str, str] | None:
    ...
    if best_slug:
        ...
        if coverage >= POWERLOG_SCHEMA_MIN_MATCH_RATIO or matches >= POWERLOG_SCHEMA_MIN_MATCH_COUNT:
...
def copy_with_companions(src_db: Path, dst_folder: Path, dst_name: str | None = None) -> Path:
    """
...
def try_sqlite_recover_to_newdb(bad_db: Path) -> Path | None:
    ...
    try:
        ...
        subprocess.run(
            ["sqlite3", str(new_db)],
            input=sql_out.read_bytes(), check=True
        )
        return new_db
    except Exception:
        return None
...
def try_sqlite_dissect(db_path: Path) -> Path | None:
    ...
    if not dissect_path or not dissect_path.exists():
        print(f"    Warning: sqlite_dissect CLI not found (looked for {SQLITE_DISSECT_PATH}).")
        return None
...
def dump_unreadable(src_file: Path):
    ...
    try:
        with src_file.open("rb") as f, out_txt.open("w", encoding="utf-8", errors="ignore") as out:
...
def process_sqlite_candidate(src_db: Path):
    ...
    if "lost_and_found" in tables_lower:
        print(f"  Warning: {src_db.name} contains 'lost_and_found' table; keeping for manual analysis only.")
        return
    ...
    for cat in dests:
        ...
        if cat == "Powerlog":
            if not powerlog_schema_slug:
                # This case should not be reached, but as a safeguard, explicitly skip copying to the root Powerlog directory if a schema match is missing.
                print(f"  [WARNING] Skipping Powerlog copy for {src_db.name} (no schema match).")
                continue
    ...
    try:
        ...
        if gzrec.exists():
            try:
                ...
                subprocess.run(
                    [str(gzrec), "-o", recovered_out.name, str(path.resolve())],  # Use absolute path for gzrecover
                    check=True,
                    capture_output=True,
                    text=True,
                ...
                if recovered_out.exists() and recovered_out.stat().st_size > 0:
                    ...
                    if fb.startswith(b"SQLite format 3"):
                        ...
                        print(f"    [SUCCESS] gzrecover recovered SQLite data to {renamed.name}")
                        ...
                        try:
                            ...
                            print(f"    Warning: Could not archive recovered temp {renamed.name}: {e}")
                        return

                    elif fb[:4] in FSEVENT_MAGICS:
                        print(f"    [SUCCESS] gzrecover recovered FSEvents data to {recovered_out.name}")
                        # move (not copy) the recovered artifact into FSEvents
                        try:
                            shutil.move(recovered_out, OUT_FSE_DIR / recovered_out.name)
                        except Exception:
                            # if move fails (e.g., cross-device), fall back to copy+unlink
                            shutil.copy2(recovered_out, OUT_FSE_DIR / recovered_out.name)
                            try: 
                                recovered_out.unlink()
                            except Exception: 
                                pass
                        return
                    ...
                    try:
                        ...
                        renamed = OUT_CORRUPT_GZ / (path.stem + "_gzrecover.sqlite")
                        recovered_out.rename(renamed)
                        print(f"    [SUCCESS] gzrecover produced an openable SQLite DB at {renamed.name}")
                        process_sqlite_candidate(renamed)
                        try:
                            archive_recovered_temp(renamed)
                        except Exception as e:
                        ...
                        print(f"    Warning: gzrecover output not recognized; kept in {OUT_CORRUPT_GZ}")
                else:
                    print("    Warning: gzrecover produced no usable output.")
            except subprocess.CalledProcessError as e2:
                print(f"    Warning: gzrecover failed ({e2.returncode}): {e2.stderr[:200]}")
        ...
        print(f"    [ERROR] The gzipped file is unrecoverable and has been moved to: {OUT_CORRUPT_GZ}")
        try:
            shutil.copy2(path, OUT_CORRUPT_GZ / path.name)
        except Exception as e:
            print(f"    [ERROR] Failed to copy {path.name} to the corrupt_gz directory: {e}")
        return
...
def walk_tree():
    for p in SRC_DIR.rglob("*"):
        ...
        # Plain file handling
        if suffix in SQLITE_SUFFIXES or True:
    ...
    for cid, name, ctype, *_ in src_colinfo:
        if name not in dst_cols:
            ...
            try:
                ...
                pass  # virtual/without rowid/etc. — best effort
...
def merge_powerlogs() -> list[Path]:
    ...
    for slug, candidates in sorted(groups.items()):
        ...
        with merge_report.open("w", encoding="utf-8") as rep:
            ...
            for db in candidates:
                try:
                    ...
                    if new_cols:
                        save_missing_schema_report(new_cols, combined_dir / "new_columns_detected.csv")
                except Exception as e:
                ...
                for tname, schema_sql in tables:
                    ...
                    try:
                        src_cols_info = cur.execute(f"PRAGMA table_info({q_tname});").fetchall()
                    except sqlite3.DatabaseError as e:
                        print(f"  Warning: Cannot inspect columns for {tname} in {db.name}: {e}")
                        continue

                    try:
                        dst_cols_info = dst_cur.execute(f"PRAGMA table_info({q_tname});").fetchall()
                    except sqlite3.DatabaseError:
                        continue
                    ...
                    if not {"ID", "timestamp"}.issubset(set(common_cols)):
                        print(f"  Warning: Skipping {tname} in {db.name} (missing ID/timestamp)")
                        continue
                    ...
                    try:
                        ...
                        try:
                            print(f"    Requerying {tname} WITHOUT indexes (index corruption suspected)")
                            rows_iter = cur.execute(f"SELECT {col_list} FROM {q_tname} NOT INDEXED;")
                        except sqlite3.DatabaseError as e:
                            print(f"  Warning: Cannot read table {tname} in {db.name}: {e}")
                    ...
                    while True:
                        ...
                        if all(v is None for v in row):
                            continue
                        if ts_idx is not None and not looks_like_unix_timestamp(row[ts_idx]):
                            continue
                        try:
                            dst_cur.execute(
        ...
        dst_conn.commit()
        dst_conn.close()
        print(f"  [SUCCESS] Powerlog merge complete for '{label}'. Report saved to {merge_report}")

        for db_name, counts in rows_per_db.items():
            if counts and all(count == 0 for count in counts):
                candidate_path = base_dir / slug / db_name if (base_dir / slug / db_name).exists() else base_dir / db_name
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        if not output_root.is_absolute():
            output_root = (input_dir / output_root).resolve()
        else:
    ...
    z_path_status = f"{gz_path} (missing)" if not gz_path or not gz_path.exists() else str(gz_path)
    dissect_path = Path(SQLITE_DISSECT_PATH) if SQLITE_DISSECT_PATH else None
    dissect_status = f"{dissect_path} (missing)" if not dissect_path or not dissect_path.exists() else str(dissect_path)
...
def process_sqlite_candidate(src_db: Path):
    ...
    if not ok:
        # Still unreadable → dump bytes to text
        print(f"  ✗ Unreadable after all recovery attempts: {src_db}")
        dump_unreadable(src_db)
        return
    tables_lower = [t.lower() for t in tables]
    if "lost_and_found" in tables_lower:
        print(f"  ⚠️  {src_db.name} contains 'lost_and_found' table; keeping for manual analysis only.")
        return
    ...
    for cat in dests:
        ...
        if not merged_ok:
            pass
        print(f"  ✓ Copied → {display_bucket}: {dst_db.name}")
...
def process_gz(path: Path):
    ...
    try:
        ...
        print(f"  ⚠️  Gzip decompression failed: {path.name} ({e})")
            ...
            try:
                ...
                if recovered_out.exists() and recovered_out.stat().st_size > 0:
                    ...
                    if fb.startswith(b"SQLite format 3"):
                        renamed = OUT_CORRUPT_GZ / (path.stem + "_gzrecover.sqlite")
                        recovered_out.rename(renamed)
                        print(f"    [SUCCESS] gzrecover recovered SQLite data → {renamed.name}")
                        process_sqlite_candidate(renamed)
                        # move the gzrecover artifact out of corrupt_gz → recovered_sqlite
                        try:
                            archive_recovered_temp(renamed)
                        ...
                        print(f"    [SUCCESS] gzrecover recovered FSEvents data → {recovered_out.name}")
                        # move (not copy) the recovered artifact into FSEvents
                        try:
                            shutil.move(recovered_out, OUT_FSE_DIR / recovered_out.name)
                        except Exception:
                    ...
                    try:
                        ...
                        renamed = OUT_CORRUPT_GZ / (path.stem + "_gzrecover.sqlite")
                        recovered_out.rename(renamed)
                        print(f"    [SUCCESS] gzrecover produced an openable SQLite DB → {renamed.name}")
                        process_sqlite_candidate(renamed)
                        try:
                            archive_recovered_temp(renamed)
                        except Exception as e:
                            print(f"    ⚠️ Could not archive {renamed.name}: {e}")
                        return

                    except Exception:
                        print(f"    ⚠️  gzrecover output not recognized; kept in {OUT_CORRUPT_GZ}")
                else:
                    print("    ⚠️  gzrecover produced no usable output.")
            except subprocess.CalledProcessError as e2:
                print(f"    ⚠️  gzrecover failed ({e2.returncode}): {e2.stderr[:200]}")
            except Exception as e3:
                print(f"    ⚠️  gzrecover error: {e3}")

        else:
            print(f"    ⚠️  gzrecover not found at {gzrec}; quarantining file.")
...
def merge_powerlogs() -> list[Path]:
    ...
    for slug, candidates in sorted(groups.items()):
        ...
        print(f"\nMerging {len(candidates)} Powerlog DB(s) for '{label}' → {combined_db.name}")
        ...
        with merge_report.open("w", encoding="utf-8") as rep:
            ...
            for db in candidates:
                try:
                    ...
                    print(f"  ⚠️  Skipping unreadable {db.name}: {e}")
                    continue
                ...
                for tname, schema_sql in tables:
                    ...
                    try:
                        ...
                        print(f"  ⚠️  Cannot inspect columns for {tname} in {db.name}: {e}")
                        continue
                    ...
                    if not {"ID", "timestamp"}.issubset(set(common_cols)):
                        print(f"  ⚠️  Skipping {tname} in {db.name} (missing ID/timestamp)")
                        continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"  ⚠️  Cannot read table {tname} in {db.name}: {e}")
                            rep.write(f"{db.name},{tname},0\n")
                            rows_per_db.setdefault(db.name, []).append(0)
                            continue
                    ...
                    while True:
                        try:
                            ...
                            print(f"  ⚠️  Row read error {db.name}:{tname}: {e}")
                            break
                        if row is None:
                            break
                        if all(v is None for v in row):
            ...
            if counts and all(count == 0 for count in counts):
                ...
                if candidate_path.exists():
                    try:
                        dest = partial_dir / db_name
                        shutil.copy2(candidate_path, dest)
                        print(f"  [WARNING] Copied fully empty Powerlog DB → {dest}")
                    except Exception as e:
                        print(f"  [WARNING] Could not copy {db_name}: {e}")
