...
class URLContext:
    ...
    def __init__(self):
        ...
        self.value_to_url: dict[int | float | str, str] = {}  # numeric value -> source URL
...
def parse_url_with_unfurl(url: str) -> URLTimestampInfo:
    ...
    # Validate URL before passing to Unfurl to avoid parsing errors
    try:
        parsed = urlparse(url)
        # Check for obviously malformed URLs
        if not parsed.scheme or not parsed.netloc:
            return _parse_url_manual(url)
        # Check for malformed port (common issue with truncated JSON)
        if parsed.port is not None and not isinstance(parsed.port, int):
            return _parse_url_manual(url)
    except (ValueError, AttributeError):
        # URL is malformed, use manual parsing
        return _parse_url_manual(url)
    ...
    try:
        ...
        unfurl_instance.add_to_queue(data_type="url", key=None, value=url)
        ...
        # Process Unfurl output (iterate over graph nodes)
        if hasattr(unfurl_instance, "graph") and unfurl_instance.graph:
            for node_id in unfurl_instance.graph.nodes():
                node = unfurl_instance.graph.nodes[node_id]
    ...
    return URLTimestampInfo(url=url, platform=platform, timestamps=timestamps, ids=ids, metadata=metadata)
...
def _extract_from_unfurl_node(node, timestamps: list, ids: list, metadata: dict):
    ...
    try:
        ...
        if any(kw in label for kw in ["time", "date", "timestamp", "created", "modified"]):
            if value and isinstance(value, (int, float)):
                timestamps.append((value, f"Unfurl: {label}"))
                metadata[label] = value

        # ID indicators
        elif any(kw in label for kw in ["id", "uid", "key", "token", "post", "message"]):
            if value:
                ids.append((value, f"Unfurl: {label}"))
                metadata[label] = value

        # Snowflake detection
        elif "snowflake" in label.lower() and value:
