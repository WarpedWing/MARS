    ...
    if isinstance(pb_data, dict):
        for key, value in pb_data.items():
            ...
            if isinstance(value, dict):
                ...
                # Recurse
                timestamps.extend(extract_timestamps_from_protobuf(value, field_path, seen))
                ...
                for i, item in enumerate(value):
                    if isinstance(item, dict):
                        timestamps.extend(extract_timestamps_from_protobuf(item, f"{field_path}[{i}]", seen))
...
def analyze_protobuf_for_timestamps(json_data: str | dict) -> dict:
    ...
    return {
        ...
        "timestamps": [{"field": field, "value": value, "human_readable": human} for field, value, human in timestamps],
...
def should_keep_protobuf(pb_data: dict) -> tuple[bool, str]:
    ...
    if field_count >= 10:
        ...
        if isinstance(pb_data, dict):
            generic_count = sum(1 for k in pb_data if isinstance(k, str) and k.startswith("f") and k[1:].isdigit())
...
if __name__ == "__main__":
    ...
    test_pb = {
        ...
        "f4": [32, 32, 34],
        "f12": {"u32": 1919512696, "f32": 4.623742314505484e30},
        "f15": {"u64": 3761972665823279650, "f64": 3.379226598219963e-57},
    }

    result = analyze_protobuf_for_timestamps(test_pb)
    logger.debug(f"Has timestamps: {result['has_timestamps']}")
    logger.debug(f"Timestamps found: {result['timestamp_count']}")
    for ts in result["timestamps"]:
        logger.debug(f"  {ts['field']}: {ts['value']} -> {ts['human_readable']}")

    # Test with timestamp example
    test_pb2 = {"state": 1, "endTime": 1592503917395, "fileSize": 392223}
