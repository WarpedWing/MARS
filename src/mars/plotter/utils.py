...
@dataclass
class PlotContext:
    """Context for chart generation with project/scan metadata."""

    project_name: str
    source_type: str  # "Exemplar" or "Candidate"
    scan_name: str  # e.g., "MARS_Test_20251122_074612" or "20251122_210537"
    db_name: str  # Database name being plotted
...
def human_time_from_epoch(
    ...
    if tzmode == "OFFSET":
        return datetime.fromtimestamp(epoch_s, tz=timezone(timedelta(minutes=offset_minutes)))
    # Local
    return datetime.fromtimestamp(epoch_s)


def _nice_bucket_seconds(cand: float) -> float:
    """Choose a friendly bucket size >= candidate seconds."""
    NICE = [
        1,
        2,
        5,
...
def compute_chart_title(
    ...
    """Compute chart title with optional project/scan context.

    Args:
        base_title: Database name or fallback title
        xs: List of x-axis epoch timestamps
        x_range: Optional (start, end) epoch filter
        tzmode: Timezone mode ("LOCAL", "UTC", "OFFSET")
        offset_minutes: UTC offset in minutes (for OFFSET mode)
        context: Optional PlotContext with project/scan metadata

    Returns:
        Tuple of (title, start_epoch, end_epoch)
    """
    ...
    if start_epoch is None and end_epoch is None:
        if context:
            return (
                f"{context.project_name} | {context.source_type}: {context.scan_name} | {context.db_name}",
                None,
                None,
            )
    ...
    start_epoch = cast("float", start_epoch)
    end_epoch = cast("float", end_epoch)
    ...
    date_part = start_str if start_str == end_str else f"{start_str} to {end_str}"

    # Build title with context if available
    if context:
        # Format: "Project | Source: Scan | DB: Date Range"
        title = f"{context.project_name} | {context.source_type}: {context.scan_name} | {context.db_name}: {date_part}"
    else:
