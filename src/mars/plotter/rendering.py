...
from mars.plotter.tui import cerr, cwarn
from mars.plotter.utils import (
    PlotContext,
    bin_time_series,
    compute_chart_title,
    ensure_outdir,
...
def _inject_page_background(html_path: Path, dark_mode: bool) -> None:
    """
    Post-process Plotly HTML to inject page background styling.

    Plotly's write_html only sets chart background, not the HTML body.
    This injects a <style> tag to match the body to the chart theme.
    """
    ...
    # Define background colors matching plotly_dark and plotly_white templates
    if dark_mode:
        bg_color = "#111111"  # Match plotly_dark paper_bgcolor
        text_color = "#f2f5fa"
    else:
        bg_color = "#ffffff"  # Match plotly_white
        text_color = "#2a3f5f"
    ...
    # Inject style right after <head> tag
    if "<head>" in content:
...
def create_overlay_figure(
    ...
    is_primary_binary = is_binary_series(primary_data)
    # Apply smoothing only to non-binary, non-bar data
    if style != "bar" and smooth_window > 1 and not is_primary_binary:
        primary_data = rolling_mean(list(primary_data), smooth_window)
    if style == "line":
        shape1 = "hv" if (step_for_binary and is_primary_binary) else "linear"
        fig.add_trace(
            go.Scatter(
                x=x_dt,
                y=primary_data,
        ...
        fig.add_trace(go.Scatter(x=x_dt, y=primary_data, mode="markers", name=primary, yaxis="y1"))
    elif style == "bar":
        fig.add_trace(go.Bar(x=x_dt, y=primary_data, name=primary, yaxis="y1", opacity=0.75))

    if len(labels) >= 2:
        secondary = labels[1]
        secondary_data = ys[secondary]
        is_secondary_binary = is_binary_series(secondary_data)
        # Apply smoothing only to non-binary, non-bar data
        if style != "bar" and smooth_window > 1 and not is_secondary_binary:
            secondary_data = rolling_mean(list(secondary_data), smooth_window)
        if style == "line":
            shape2 = "hv" if (step_for_binary and is_secondary_binary) else "linear"
            fig.add_trace(
                go.Scatter(
                    x=x_dt,
                    y=secondary_data,
    ...
    fig.update_layout(
        ...
        xaxis={
            ...
            "zeroline": False,
            "automargin": True,
            "rangeslider": {"visible": True},  # Time slider for zoom/pan
        },
        yaxis={
            "title": labels[0],
            "showgrid": True,
...
def render_stacked_lines(
    ...
    for i, panel in enumerate(panels, start=1):
        ...
        hover_labels = panel.get("hover_labels")  # Optional list of label strings
        all_xs.extend(xs)
        is_bool = is_binary_series(ys)
        yplot = ys if (is_bool or smooth_window <= 1) else rolling_mean(list(ys), smooth_window)
        x_dt = [human_time_from_epoch(x, tzmode, offset_minutes) for x in xs]
        shape = "hv" if (step_for_binary and is_bool) else "linear"

        # Build trace with optional hover labels
        trace_kwargs: dict = {
            "x": x_dt,
            "y": yplot,
            "mode": "lines+markers",
        ...
        if hover_labels:
            # Replace None with empty string to avoid showing literal "%{hovertext}"
            trace_kwargs["hovertext"] = [lbl if lbl else "" for lbl in hover_labels]
            trace_kwargs["hovertemplate"] = "%{hovertext}<br>%{x}<br>%{y}<extra></extra>"

        fig.add_trace(go.Scatter(**trace_kwargs), row=i, col=1)

    chart_title, start_epoch, end_epoch = compute_chart_title(
        base_title, all_xs, x_range, tzmode, offset_minutes, context=context
    )
    fig.update_layout(
        title=chart_title,
        showlegend=True,
    ...
    # Add spike lines for precise value reading
    fig.update_xaxes(showspikes=True, spikemode="across", spikesnap="cursor")
    fig.update_yaxes(showspikes=True, spikemode="across", spikesnap="cursor")

    # Add range slider to bottom x-axis only
    ...
    annotations = list(getattr(fig.layout, "annotations", []))
    annotations.append(
        {
            "xref": "paper",
            "yref": "paper",
    ...
    _inject_page_background(html_path, dark_mode)
    return html_path
...
def render_plot_html(
    ...
    *,
    ...
    x_range: tuple[float | None, float | None] | None = None,
    context: PlotContext | None = None,
    hover_labels: list[str | None] | None = None,
    dark_mode: bool = False,
) -> Path:
    """
    ...
    if style != "bar" and smooth_window and smooth_window > 1:
        ys_used = {k: (v if binary_map.get(k, False) else rolling_mean(v, smooth_window)) for k, v in ys_used.items()}
    ...
    chart_title, start_epoch, end_epoch = compute_chart_title(
        base_title, xs_used, x_range, tzmode, offset_minutes, context=context
    )
    ...
    if style == "line" and y2_col and (line_layout.lower().startswith("stack")):
        ...
        shape1 = "hv" if (step_for_binary and binary_map.get(series[0], False)) else "linear"
        fig.add_trace(
            go.Scatter(
                x=x_dt,
                y=ys_used[series[0]],
        ...
        shape2 = "hv" if (step_for_binary and binary_map.get(y2_col, False)) else "linear"
        fig.add_trace(
            go.Scatter(
                x=x_dt,
                y=ys_used[y2_col],
        ...
        for ann in getattr(fig.layout, "annotations", []):
            if ann.text in panel_titles:
                ann.update(x=0.5, xanchor="center", y=1.08, yanchor="bottom", font={"size": 14})
        ...
        try:
            ...
            plotly_plot(fig, filename=str(html_path), auto_open=True, include_plotlyjs=True)
        if not html_path.exists():
    ...
    # Build hover kwargs if labels are provided
    hover_kwargs: dict = {}
    if hover_labels:
        # Replace None with empty string to avoid showing literal "%{hovertext}"
        hover_kwargs["hovertext"] = [lbl if lbl else "" for lbl in hover_labels]
        hover_kwargs["hovertemplate"] = "%{hovertext}<br>%{x}<br>%{y}<extra></extra>"
    ...
    y2_on_y2_axis = y2_col and y2_col in ys_used and (style != "bar" or bar_mode.lower().startswith("over"))
    ...
    fig.update_layout(
        ...
        xaxis={
            ...
            "zeroline": False,
            "automargin": True,
            "rangeslider": {"visible": True},  # Range slider for zoom/pan
        },
        yaxis={
            "title": series[0],
            "showgrid": True,
    ...
    if y2_on_y2_axis:
        ...
        fig.update_layout(barmode=("overlay" if bar_mode.lower().startswith("over") else "group"))
    ...
    try:
        ...
        try:
            ...
            plotly_plot(fig, filename=str(html_path), auto_open=False, include_plotlyjs=True)
