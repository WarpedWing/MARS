...
INSTAGRAM_EPOCH = 1388534400  # 2014-01-01 (rough Instagram ID epoch)
...
def detect_sequential_id(
    value: int, all_values: list[int], tolerance: int = 1000
) -> IDDetectionResult:
    """
...
def detect_instagram_id(value: int) -> IDDetectionResult:
    """
    Detect Instagram media IDs.

    Instagram uses modified Snowflake-like IDs but with different epoch.
    """
    # Instagram IDs are typically 18-19 digits
    if not (18 <= len(str(abs(value))) <= 19):
        return IDDetectionResult(False, None, 0.0, "Wrong digit count")

    # Try Instagram epoch
    try:
        # Instagram IDs encode timestamp but with custom epoch
        # This is a heuristic - Instagram's exact format varies
        timestamp_sec = (value >> 22) + INSTAGRAM_EPOCH

        dt = datetime.fromtimestamp(timestamp_sec, tz=UTC)
        if not (datetime(2010, 1, 1, tzinfo=UTC) <= dt <= datetime(2030, 1, 1, tzinfo=UTC)):
            return IDDetectionResult(False, None, 0.0, "Timestamp out of range")

        return IDDetectionResult(
            is_id=True,
            id_type="instagram_id",
            confidence=0.7,
            description=f"Instagram media ID (approx: {dt.strftime('%Y-%m-%d')})",
            embedded_timestamp=timestamp_sec
        )

    except Exception:
        return IDDetectionResult(False, None, 0.0, "Parse error")
...
def extract_snowflake_timestamp(snowflake_id: int) -> float | None:
    """
    Extract embedded timestamp from a Snowflake ID.

    Returns Unix timestamp (seconds) or None if invalid.
    """
    result = detect_snowflake_id(snowflake_id)
    if result.is_id and result.embedded_timestamp:
        return result.embedded_timestamp
    return None
